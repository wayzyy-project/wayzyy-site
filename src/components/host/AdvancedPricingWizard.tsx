...
const STEP_HELP: string[] = [
  "See how your price compares with similar stays on Wayzyy and other booking platforms, so you can pick a rate that gets booked. Nothing on this step changes your prices.",
  "Choose which nights this pricing applies to — a whole month, the first or second half of a month, or the exact dates you picked on the calendar. You can select several months at once.",
  "Set your normal weekday rate and your weekend rate for Friday and Saturday nights. The weekend rate can be a fixed amount or a percentage above the weekday rate.",
  "Optionally raise or lower each month. The percentage is added on top of your weekday and weekend rates, and applies only to the nights you selected in that month — not the whole calendar month. Use 0% to leave a month unchanged.",
      ...
      <div className="liquid-glass flex max-h-[92vh] w-full max-w-2xl flex-col rounded-t-3xl border border-white/15 bg-black/80 sm:rounded-3xl">
        <header className="flex items-start justify-between gap-4 border-b border-white/10 p-5">
          <div>
            ...
            <div className="mt-1 flex items-center gap-1.5">
              <h2 className="font-display text-lg font-bold text-white">{STEPS[step]}</h2>
              <StepInfo text={STEP_HELP[step]} />
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
                  ...
                  {PARTS.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="block text-[11px] text-white/70">{p.hint}</span>
                      </button>
                    );
                  })}
                </div>
                {!selection.length && <p className="mt-2 text-[11px] text-white/70">To price specific dates, close this and select them on the calendar first.</p>}
              </div>

              {part !== "selection" && (
                <div>
                  <p className="mb-2 text-sm font-semibold text-white">Which months? <span className="font-normal text-white/70">Pick as many as you like</span></p>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-[11px] text-white/70">Your base nightly price. Your current standard rate is {basePrice ? formatINR(basePrice) : "not set"}.</p>
          ...
          {step === 3 && (
            <MonthlyStep
              months={periodMonths}
              nightsByMonth={Object.fromEntries(preview.map((r) => [r.month, r.nights]))}
              plan={plan}
              setPlan={setPlan}
              insights={insights}
            />
          ...
          {step === 4 && (
            <div className="space-y-4">
              <p className="text-xs text-white/80">
                Here's what each night will cost after your weekday and weekend rates and any monthly change. Nothing is
                saved until you tap Apply.
              </p>
...
function InsightsStep({ insights, state, basePrice, onUse }: { insights: PricingInsights | null; state: "loading" | "ready" | "error"; basePrice: number | null; onUse: () => void }) {
  if (state === "loading") {
    return <div className="flex flex-col items-center gap-2 py-16 text-xs text-white/70"><Loader2 className="h-5 w-5 animate-spin" /> Comparing you with similar stays…</div>;
  }
    ...
    <div className="space-y-4">
      ...
      {ref && (
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <p className="text-xs text-white/70">Similar {insights.property.bedrooms}-bedroom stays in {insights.property.locality} on {refName}</p>
      ...
      {season && (
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <p className="text-[11px] text-white/70">Average nightly rate on booking platforms each month vs the yearly average</p>
            ...
            <div className="flex h-28 min-w-[420px] items-center gap-1">
              {season.map((s) => (
                <div key={s.month} className="flex h-full flex-1 flex-col items-center" title={`${MONTH_NAMES[s.month - 1]}: ${formatPct(s.pct)} (avg ${formatINR(s.adr)})`}>
                  ...
                  <span className="mt-1 text-[9px] text-white/70">{MONTH_NAMES[s.month - 1].slice(0, 3)}</span>
                </div>
              ))}
            </div>
          </div>
...
function MonthlyStep({ months, nightsByMonth, plan, setPlan, insights }: { months: string[]; nightsByMonth: Record<string, number>; plan: PricingPlan; setPlan: (p: PricingPlan) => void; insights: PricingInsights | null }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-white/80">
        Raise or lower each month on top of your weekday and weekend rates. Each change applies only to the nights you
        selected in that month, shown below. Use 0% to keep a month as it is.
      </p>
      {hasSuggestions && (
        ...
        {months.map((m) => {
          ...
          return (
            <li key={m} className="flex flex-wrap items-center gap-3 rounded-xl border border-white/10 px-3 py-2.5">
              <span className="w-28 text-sm font-medium text-white">
                {monthLabel(m)}
                <span className="block text-[10px] font-normal text-white/60">{nightsByMonth[m] ?? 0} night{(nightsByMonth[m] ?? 0) === 1 ? "" : "s"} selected</span>
              </span>
              ...
              {suggested[m] != null && suggested[m] !== pct && (
                <span className="w-full text-[10px] text-white/70">Market suggests {formatPct(suggested[m])}</span>
              )}
            </li>
          );
        })}
