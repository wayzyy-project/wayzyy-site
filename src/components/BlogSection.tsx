...
export function BlogSection() {
  const whyWeBuiltPost = blogPosts.find((p) => p.slug === "why-we-decided-to-build-wayzyy-differently") ?? blogPosts[0];
  const platformComparisonPost = blogPosts.find((p) => p.slug === "airbnb-vs-booking-vs-wayzyy") ?? blogPosts[1];

  const featuredPosts = [
    {
      ...whyWeBuiltPost,
      badge: "Founder's Notes",
      icon: "✍️",
      subtitle: "Our founder's perspective on why we chose a flat, prepaid credit model over Airbnb's high commission fees.",
    },
    {
      ...platformComparisonPost,
      badge: "Platform Comparison",
      icon: "📊",
      subtitle: "An honest, side-by-side comparison of fees, discovery, and long-term profitability for hosts.",
    },
  ];
              ...
              >
                <div className="relative aspect-video w-full overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-4 left-4 rounded-full border border-ember/40 bg-black/75 backdrop-blur-md px-3 py-1 text-[11px] font-semibold text-white shadow-md">
                    {post.icon} {post.badge}
                  </div>
                </div>

                <div className="flex flex-col justify-between flex-1 p-6 sm:p-8 space-y-4">
                  <div className="space-y-3">
                    <h3 className="font-display text-xl sm:text-2xl text-foreground transition-colors group-hover:text-ember leading-snug">
                      {post.title}
                    </h3>
